...
from train_utils import train, eval, Evaluator
...
from mp.MFNN_graph import MFNN
...
def main(args):
    ...
    if args.model.startswith('gin') or args.model in ['MFNN']:  # load graph dataset
        graph_list, train_ids, val_ids, test_ids, num_classes = load_graph_dataset(
            args.dataset, fold=args.fold, petalType= args.petalType, max_petal_dim=args.max_petal_dim)

        # 将我们的代码嵌入进去，HL保存到graph_list中
        ...
        train_loader = DataLoader(train_graphs, batch_size=args.batch_size,
                                     shuffle=True, num_workers=args.num_workers)
        valid_loader = DataLoader(val_graphs, batch_size=args.batch_size,
                                     shuffle=False, num_workers=args.num_workers)
    ...
    # elif args.model == 'OurHiGCN':
    #     model = OurHiGCN(args.max_petal_dim,
    ...
    #                  readout=args.readout,                    # readout
    #     ).to(device)
    elif args.model == 'MFNN':
        model = MFNN(args.max_petal_dim,
                         num_features,  # num_input_features
                         args.num_layers,  # num_layers
                         args.emb_dim,  # hidden
                         num_classes,  # num_classes
    ...
    # elif args.model == 'OurHiGCN_old':
    #     model = OurHiGCN_old(args.max_petal_dim, #max simplix dimension
    #                  num_features,  # num_input_features
    #                  10,    #K
    #                  args.emb_dim,  # hidden
    #                  num_classes,  # num_classes
    #                  dropout=args.drop_rate,  # dropout rate
    #                  alpha=0.5, #alpha
    #                  readout=args.readout,  # readout
    #                  nonlinearity=args.nonlinearity,  # nonlinearity
    #
    ...
    result_folder = os.path.join(
        args.result_folder, f'{args.dataset}-{args.exp_name}', f'seed-{args.seed}')
    if args.fold is not None:
        best_val_acc=max(curves['val'])
        result_folder = os.path.join(result_folder, f'fold-{args.fold}-{best_val_acc}')
    if not os.path.exists(result_folder):
        os.makedirs(result_folder)
    filename = os.path.join(result_folder, 'results.txt')
