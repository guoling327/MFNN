...
def gen_HL2(name, root, graph_list, petalType, petal_dim):
    """
    petalType: simplex, ring
    petal_dim: 单纯形的时候对应的是其阶数，如=3表示四面体；ring时表示圈的大小-1
    max_petal_dim: 找单纯形和cycle时限制的阶数（初始找大一点后面就不用再找了）
    ...
    # s_path = os.path.join(root, name, name + '_e.pt')
    # u_path = os.path.join(root, name, name + '_u.pt')
    ...
    if os.path.exists(HL_path):
        ...
        # else:
        #
        #             for i in range(petal_dim):
        #                 HL_sparse_tensor = HL[i + 1]  # 这里选择HL中的第一个稀疏张量进行SVD分解
        #                 # 将稀疏张量转换为稠密张量
        #                 dense_tensor = HL_sparse_tensor.to_dense()
        #                 U, S, V = torch.svd(dense_tensor)
        #
        #                 e.append(S.to(device))
        #                 u.append(U.to(device))
        #
        #             torch.save(e, s_path)
        #             torch.save(u, u_path)
    ...
    for idx in range(len(graph_list)):
        graph_list[idx].HL = {order: HL[idx][order] for order in range(1, petal_dim+1)}
        for order in range(1, petal_dim+1):
            HL_sparse_tensor =HL[idx][order]
            dense_tensor = HL_sparse_tensor.to_dense()
